...
(function () {
  ...
  var DICT = {
    ...
    'ABC-Analyse (Forecast)': 'ABC analysis (forecast)',
    'ABC-Analyse (Forecast-Mengen)': 'ABC analysis (forecast quantities)',
    'Alle SKU exportieren (Excel)': 'Export all SKUs (Excel)',
    'Andere Datenbasis als die ABC-Klasse in der Artikel-Standortanalyse (dort: SKU-View/Sales-History-ESU, nicht Forecast-Stückzahl) — beide Klassifizierungen können für denselben Artikel unterschiedlich ausfallen.': 'A different data basis than the ABC class in the article location analysis (there: SKU View/Sales-History ESU, not forecast unit quantity) — the two classifications can come out differently for the same article.',
    'Je Artikel: Σ Menge (Stück) über alle geladenen Forecast-Zeilen (alle DCs/Perioden/Kategorien), absteigend sortiert. A = bis 80 % kumulierter Menge, B = bis 95 %, C = restliche 5 %.': 'Per article: Σ quantity (units) across all loaded forecast rows (all DCs/periods/categories), sorted descending. A = up to 80% cumulative quantity, B = up to 95%, C = the remaining 5%.',
    'Klasse {0}': 'Class {0}',
    'Klassifiziert jeden Artikel nach seinem Anteil an der gesamten Forecast-Menge (Stück, über alle geladenen DCs/Perioden/Kategorien): A = Top-Artikel bis 80 % kumulierter Menge, B = bis 95 %, C = die restlichen, langsam drehenden Artikel. Andere Datenbasis als die ABC-Klasse in der Artikel-Standortanalyse (dort SKU-View/Sales-History-ESU statt Forecast-Stückzahl).': 'Classifies every article by its share of the total forecast quantity (units, across all loaded DCs/periods/categories): A = top articles up to 80% cumulative quantity, B = up to 95%, C = the remaining, slow-moving articles. A different data basis than the ABC class in the article location analysis (there: SKU View/Sales-History ESU instead of forecast unit quantity).',
    'Kum. Anteil': 'Cum. share',
    'Menge (Stück)': 'Quantity (units)',
    '{0} Artikel insgesamt{1}.': '{0} articles in total{1}.',
    ' — die ersten {0} nach Menge angezeigt': ' — first {0} shown by quantity',
    'C-Artikel zentral zugewiesen an': 'C articles centrally assigned to',
    'C-Artikel zentral zuweisen': 'Assign C articles centrally',
    'DC-Gesamtübersicht (ABC-Analyse Forecast)': 'DC overview (ABC analysis forecast)',
    'DC-Gesamtübersicht (Ziel-Reichweite)': 'DC overview (target coverage)',
    'Dieselbe Formel wie Zyklusbestand, hier je Artikel statt je DC — ohne artikelbezogenen Sicherheitsbestand (dafür wäre eine monatliche Schwankungsreihe je Artikel nötig). Die Reichweite ist in diesem Bericht frei einstellbar (Standard: globale Ziel-Reichweite aus Daten & Import) und unabhängig von der globalen Einstellung.': 'The same formula as cycle stock, here per article instead of per DC — without an article-level safety stock (that would need a monthly variance series per article). Coverage is freely adjustable in this report (default: the global target coverage from Data & Import) and independent of that global setting.',
    'Direkt aus der eigenen DC-Zuordnung des Forecasts (keine Distrikt-Näherung nötig, da der Forecast bereits je DC vorliegt). Ein DC, wenn ein Standort bereits dominiert; mehrere, wenn sich die Menge real auf mehrere Standorte verteilt. Optional überschreibbar: alle C-Artikel lassen sich manuell einem einzigen, frei gewählten DC zuweisen (Konsolidierung der langsam drehenden Artikel an einem Standort).': 'Straight from the forecast\'s own DC assignment (no district approximation needed, since the forecast is already available per DC). One DC when a single site already dominates, several when the quantity is genuinely split across multiple sites. Optionally overridable: all C articles can be manually assigned to a single, freely chosen DC (consolidating the slow-moving articles at one site).',
    'Empfohlene DC(s) je Artikel: direkt aus der eigenen DC-Zuordnung des Forecasts abgeleitet (keine Distrikt-Näherung nötig). Angezeigt wird die kleinste Anzahl Standorte — von der Menge her absteigend sortiert —, deren Summe mindestens 80 % der Artikel-Gesamtmenge erreicht: ein Standort, wenn er bereits dominiert; mehrere, wenn sich die Menge real auf mehrere Standorte verteilt. C-Artikel lassen sich unten optional manuell einem einzigen DC zuweisen.': 'Recommended DC(s) per article: derived straight from the forecast\'s own DC assignment (no district approximation needed). Shows the smallest number of sites — sorted descending by quantity — whose sum reaches at least 80% of the article\'s total quantity: one site if it already dominates, several if the quantity is genuinely split across multiple sites. C articles can optionally be assigned to a single DC manually below.',
    'Je DC: Σ Ziel-Paletten aller Artikel, verteilt auf deren empfohlene(s) DC(s) (Anteile auf 100 % je Artikel renormiert).': 'Per DC: Σ target pallets of all articles, distributed across their recommended DC(s) (shares renormalized to 100% per article).',
    'Keine Zentralisierung': 'No centralization',
    'Menge (Ziel-Reichweite)': 'Quantity (target coverage)',
    'Menge/Paletten zur Ziel-Reichweite (ABC-Analyse Forecast)': 'Quantity/pallets at target coverage (ABC analysis forecast)',
    'PAL (Ziel-Reichweite)': 'PAL (target coverage)',
    'Wert': 'Value',
    'Zeigt, welchen Ziel-Palettenbestand jeder Standort bräuchte, wenn jeder Artikel exakt gemäß seiner empfohlenen DC-Zuordnung (inkl. einer eventuellen manuellen C-Artikel-Zentralisierung) bevorratet würde.': 'Shows the target pallet stock each site would need if every article were stocked exactly per its recommended DC assignment above (including any manual C-article centralization).',
    'Ziel-Menge = Bedarf/Tag(Artikel) × 30,44 × Reichweite(Monate) × Sicherheitsaufschlag; Ziel-Paletten analog mit Paletten statt Menge.': 'Target quantity = demand/day(article) × 30.44 × coverage(months) × safety surcharge; target pallets analogously with pallets instead of quantity.',
    'Ziel-Paletten (Reichweite)': 'Target pallets (coverage)',
    'Ziel-Paletten je DC, wenn jeder Artikel gemäß seiner empfohlenen DC-Zuordnung oben (inkl. C-Artikel-Zentralisierung, falls gewählt) bevorratet wird.': 'Target pallets per DC if every article is stocked per its recommended DC assignment above (including C-article centralization, if chosen).',
    'Empfohlene DC(s)': 'Recommended DC(s)',
    'Empfohlene DC(s) je Artikel (Forecast-basiert)': 'Recommended DC(s) per article (forecast-based)',
    'Je Artikel die Forecast-Menge je DC absteigend sortiert; die kleinste Anzahl DCs von oben, deren Summe ≥ 80 % der Artikel-Gesamtmenge erreicht, wird empfohlen.': 'Per article, the forecast quantity per DC sorted descending; the smallest number of DCs from the top whose sum reaches ≥ 80% of the article\'s total quantity is recommended.',
    'Direkt aus der eigenen DC-Zuordnung des Forecasts (keine Distrikt-Näherung nötig, da der Forecast bereits je DC vorliegt). Ein DC, wenn ein Standort bereits dominiert; mehrere, wenn sich die Menge real auf mehrere Standorte verteilt.': 'Straight from the forecast\'s own DC assignment (no district approximation needed, since the forecast is already available per DC). One DC when a single site already dominates; several when the quantity is genuinely split across multiple sites.',
    'Empfohlene DC(s) je Artikel: direkt aus der eigenen DC-Zuordnung des Forecasts abgeleitet (keine Distrikt-Näherung nötig). Angezeigt wird die kleinste Anzahl Standorte — von der Menge her absteigend sortiert —, deren Summe mindestens 80 % der Artikel-Gesamtmenge erreicht: ein Standort, wenn er bereits dominiert; mehrere, wenn sich die Menge real auf mehrere Standorte verteilt.': 'Recommended DC(s) per article: derived straight from the forecast\'s own DC assignment (no district approximation needed). Shows the smallest number of sites — sorted descending by quantity — whose sum reaches at least 80% of the article\'s total quantity: one site if it already dominates, several if the quantity is genuinely split across multiple sites.',
